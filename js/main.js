...
// ── Constantes ────────────────────────────────────────────────────────────
const NOMBRES_NIVEL = [
  "", "Novato", "Jugador", "Veterano", "Experto", "Élite",
  "Leyenda", "Maestro", "Campeón", "Gran Maestro", "Supremo"
];
...
const LOGROS = [
  ...
  { id: "nivel_3",         icon: "⭐", nombre: "Veterano",           desc: "Llegaste al nivel 3",                  condicion: u => u.nivel >= 3 },
  { id: "nivel_5",         icon: "🔥", nombre: "Élite",       desc: "Llegaste al nivel 5",                  condicion: u => u.nivel >= 5 },
  { id: "nivel_10",        icon: "💎", nombre: "Supremo",       desc: "Alcanzaste el nivel máximo",           condicion: u => u.nivel >= 10 },
  ...
  { id: "cien_monedas",    icon: "🪙", nombre: "Ahorrista",          desc: "Tenés 100 monedas",                 condicion: u => u.monedas >= 100 },
...
async function actualizarPerfil() {
  ...
  if (typeof Social !== 'undefined' && u.uid) {
    ...
    // Listeners globales para mostrar notif badge en header
    if (!window._socialListenersSet) {
      window._socialListenersSet = true;
      // Para detectar mensajes NUEVOS (no notificar los que ya estaban al cargar)
      let _seenChats = null;
      let _myUidCache = u.uid;
      try {
        Social.onSolicitudesEntrantes(reqs => {
          updateSocialNotifDot(reqs.length > 0);
          // Notificar nuevas solicitudes solo si no estamos en social.html
          if (!location.pathname.endsWith('/social.html') && reqs.length > 0) {
            const last = reqs[0];
            if (last && !window._lastReqId || window._lastReqId !== last.fromUid) {
              window._lastReqId = last.fromUid;
              showSocialToast(`👥 ${last.fromNombre || 'Alguien'} te envió una solicitud de amistad`, 'social.html');
            }
          }
        });
        Social.onInvitaciones(invites => {
          updateSocialNotifDot(invites.length > 0);
          if (!location.pathname.endsWith('/social.html') && invites.length > 0) {
            const last = invites[0];
            if (last && (!window._lastInvId || window._lastInvId !== (last.id||last.juegoId))) {
              window._lastInvId = (last.id || last.juegoId);
              const url = `pages/juego.html?id=${last.juegoId}` + (last.roomId ? `&room=${last.roomId}` : '');
              showSocialToast(`🎮 ${last.fromNombre || 'Alguien'} te invita a ${last.juegoNombre || 'jugar'}`, url);
            }
          }
        });
        Social.onChatsActivos(chats => {
          // Contar chats con mensajes no leídos
          let unread = 0;
          (chats||[]).forEach(c => {
            const partnerUid = (c.participants||[]).find(p => p !== Social.uid);
            if (partnerUid && c.lastSenderUid && c.lastSenderUid !== Social.uid) {
              const lastRead = (c.lastRead && c.lastRead[Social.uid]) || 0;
              const lastTs = c.lastTs || 0;
              const tsMs = (typeof lastTs === 'object' && lastTs.seconds) ? lastTs.seconds * 1000 : (typeof lastTs === 'number' ? lastTs : 0);
              if (tsMs > lastRead) unread++;
            }
          });
          updateSocialNotifDot(unread > 0);

          // Notificación de mensaje nuevo (toast) si no estamos en social.html
          if (_seenChats !== null && !location.pathname.endsWith('/social.html')) {
            (chats||[]).forEach(c => {
              const prev = _seenChats[c.id];
              const tsMs = (typeof c.lastTs === 'object' && c.lastTs && c.lastTs.seconds) ? c.lastTs.seconds * 1000 : (typeof c.lastTs === 'number' ? c.lastTs : 0);
              const prevTs = prev ? prev.tsMs : 0;
              if (tsMs > prevTs && c.lastSenderUid && c.lastSenderUid !== _myUidCache) {
                // Mensaje nuevo de otra persona
                const partnerUid = (c.participants||[]).find(p => p !== _myUidCache);
                const partnerName = (c.lastSenderNombre) || partnerUid || 'Amigo';
                showSocialToast(`💬 ${partnerName}: ${(c.lastMsg||'(mensaje)').slice(0,80)}`, 'social.html');
              }
            });
          }
          _seenChats = {};
          (chats||[]).forEach(c => {
            const tsMs = (typeof c.lastTs === 'object' && c.lastTs && c.lastTs.seconds) ? c.lastTs.seconds * 1000 : (typeof c.lastTs === 'number' ? c.lastTs : 0);
            _seenChats[c.id] = { tsMs };
          });
        });
      } catch(e){ console.warn('[Social listeners]', e); }
    }
  }
...
// Mostrar/ocultar dot de notificación social
function updateSocialNotifDot(hasNotif) {
  const dot = document.getElementById('social-notif-dot');
  if (!dot) return;
  dot.style.display = hasNotif ? 'inline-block' : 'none';
}

// Toast de notificación social (mensaje, invitación, solicitud) — clickeable
function showSocialToast(text, gotoUrl) {
  // Crear contenedor si no existe
  let host = document.getElementById('social-toast-host');
  if (!host) {
    host = document.createElement('div');
    host.id = 'social-toast-host';
    host.style.cssText = 'position:fixed;top:80px;right:16px;z-index:9999;display:flex;flex-direction:column;gap:8px;pointer-events:none';
    document.body.appendChild(host);
  }
  const t = document.createElement('div');
  t.style.cssText = 'background:linear-gradient(135deg,#1a1a2e,#222244);border:1px solid rgba(0,170,255,0.4);color:#fff;padding:12px 16px;border-radius:10px;font-family:"Rajdhani",sans-serif;font-size:13px;font-weight:600;box-shadow:0 6px 20px rgba(0,0,0,0.5),0 0 12px rgba(0,170,255,0.15);cursor:pointer;pointer-events:auto;max-width:340px;animation:slideInToast .3s ease;backdrop-filter:blur(8px);';
  t.textContent = text;
  // Animación CSS si no está
  if (!document.getElementById('social-toast-style')) {
    const s = document.createElement('style');
    s.id = 'social-toast-style';
    s.textContent = '@keyframes slideInToast{from{transform:translateX(100%);opacity:0}to{transform:translateX(0);opacity:1}}@keyframes fadeOutToast{to{transform:translateX(100%);opacity:0}}';
...
async function crearCardHTML(juego, indice) {
  ...
  const tieneVideo = juego.preview_video;

  return `
    <a class="game-card" href="pages/juego.html?id=${juego.id}"
       style="animation-delay:${indice*0.04}s"
       ${tieneVideo ? `data-preview-video="${juego.preview_video}"` : (tienePreview ? `data-preview="pages/${juego.embed_url}"` : "")}
...
function iniciarPreview(card) {
  const videoUrl = card.dataset.previewVideo;
  const iframeUrl = card.dataset.preview;
  if (!videoUrl && !iframeUrl) return;
  ...
  previewTimer = setTimeout(() => {
    ...
    if (videoUrl) {
      // Modo video: más rápido, mejor calidad, sin lag de iframe
      if (!wrap.querySelector('video')) {
        ...
        vid.onerror = () => {
          // Video no encontrado - volver al thumbnail estático
...
async function renderizarRankingGlobal() {
  ...
  contenedor.innerHTML = lista.map((p, i) => {
    ...
    // Mostrar foto de perfil si tiene, si no el emoji avatar
    const avatarHtml = p.fotoPerfil
      ? `<img src="${p.fotoPerfil}" style="width:38px;height:38px;border-radius:50%;object-fit:cover;border:2px solid rgba(255,102,0,0.35)" onerror="this.style.display='none'">`
      : `<span style="font-size:26px">${p.avatar || "😊"}</span>`;
    return `
      <div class="ranking-fila ${esSoy ? "ranking-yo" : ""}"
           onclick="verPerfilUsuario('${p.uid}')"
           style="cursor:pointer;transition:background .15s"
           onmouseenter="this.style.background='rgba(255,102,0,0.06)'"
           onmouseleave="this.style.background=''">
        <div class="ranking-pos">${medal}</div>
        <div class="ranking-avatar" style="display:flex;align-items:center;justify-content:center;width:42px;height:42px;border-radius:50%;overflow:hidden;background:rgba(255,255,255,0.05)">${avatarHtml}</div>
...
async function verPerfilUsuario(uid) {
  const miUid = Storage.auth.currentUser?.uid;
  if (uid === miUid) { irSeccion("perfil"); return; }
  ...
  if (!modal) {
    ...
    modal.style.cssText = "position:fixed;inset:0;background:rgba(0,0,0,0.75);z-index:999;display:flex;align-items:center;justify-content:center;padding:16px";
    modal.innerHTML = `<div style="background:#111120;border:1px solid rgba(255,102,0,0.3);border-radius:18px;padding:24px;max-width:340px;width:100%;position:relative;text-align:center">
      <button onclick="document.getElementById('modal-perfil-usuario').remove()" style="position:absolute;top:12px;right:12px;background:rgba(255,255,255,0.05);border:1px solid rgba(255,255,255,0.1);color:#aaa;padding:4px 10px;border-radius:8px;cursor:pointer">✕</button>
      <div id="mpf-avatar" style="font-size:52px;margin-bottom:8px">😊</div>
      <div id="mpf-nombre" style="font-size:20px;font-weight:700;margin-bottom:4px">Cargando...</div>
      <div id="mpf-rango" style="font-size:12px;color:#ff6600;letter-spacing:1px;margin-bottom:14px"></div>
      <div style="display:grid;grid-template-columns:1fr 1fr 1fr;gap:8px;margin-bottom:16px">
        <div style="background:rgba(255,255,255,0.04);border-radius:10px;padding:10px">
          <div id="mpf-xp" style="font-size:18px;font-weight:700;color:#00aaff">0</div>
          <div style="font-size:10px;color:#666;letter-spacing:1px">XP</div>
        </div>
        <div style="background:rgba(255,255,255,0.04);border-radius:10px;padding:10px">
          <div id="mpf-nivel" style="font-size:18px;font-weight:700;color:#ff6600">1</div>
          <div style="font-size:10px;color:#666;letter-spacing:1px">NIVEL</div>
        </div>
        <div style="background:rgba(255,255,255,0.04);border-radius:10px;padding:10px">
          <div id="mpf-juegos" style="font-size:18px;font-weight:700;color:#39ff14">0</div>
          <div style="font-size:10px;color:#666;letter-spacing:1px">PARTIDAS</div>
        </div>
      </div>
      <div id="mpf-acciones" style="display:flex;gap:8px;justify-content:center"></div>
    </div>`;
  ...
  try {
    const doc = await firebase.firestore().doc(`usuarios/${uid}`).get();
    const p = doc.exists ? doc.data() : null;
    if (!p) { modal.querySelector("#mpf-nombre").textContent = "Usuario no encontrado"; return; }

    const avatarEl = modal.querySelector("#mpf-avatar");
    if (p.fotoPerfil) {
      avatarEl.innerHTML = `<img src="${p.fotoPerfil}" style="width:72px;height:72px;border-radius:50%;object-fit:cover;border:3px solid rgba(255,102,0,0.4)">`;
    } else {
      avatarEl.textContent = p.avatar || "😊";
    }
    modal.querySelector("#mpf-nombre").textContent = p.nombre || "Usuario";
    modal.querySelector("#mpf-rango").textContent = NOMBRES_NIVEL[p.nivel] || "";
    modal.querySelector("#mpf-xp").textContent = (p.xp || 0).toLocaleString();
    modal.querySelector("#mpf-nivel").textContent = p.nivel || 1;
    modal.querySelector("#mpf-juegos").textContent = p.juegos_jugados || 0;

    // Acciones: ir a chat si es amigo
    if (typeof Social !== "undefined" && Social.uid) {
      const estado = await Social.estadoAmistad(uid);
      let btnHtml = "";
      if (estado === "friends") {
        btnHtml = `<button onclick="window.location.href='pages/social.html'" style="background:rgba(0,170,255,0.15);border:1px solid rgba(0,170,255,0.4);color:#00aaff;padding:8px 16px;border-radius:8px;cursor:pointer;font-weight:700">💬 Chat</button>`;
      } else if (estado === "none") {
        btnHtml = `<button onclick="Social.enviarSolicitud('${uid}').then(()=>this.textContent='✅ Enviado').catch(()=>{})" style="background:rgba(255,102,0,0.15);border:1px solid rgba(255,102,0,0.4);color:#ff6600;padding:8px 16px;border-radius:8px;cursor:pointer;font-weight:700">+ Agregar amigo</button>`;
      } else if (estado === "pending_out") {
        btnHtml = `<span style="color:#666;font-size:13px">⏳ Solicitud enviada</span>`;
      }
      modal.querySelector("#mpf-acciones").innerHTML = btnHtml;
    }
  } catch(e) {
